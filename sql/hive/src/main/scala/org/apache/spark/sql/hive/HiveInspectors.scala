  ...
  def unwrapperFor(objectInspector: ObjectInspector): Any => Any =
    objectInspector match {
      ...
      case poi: WritableConstantDateObjectInspector =>
        val constant = DateTimeUtils.fromJavaDate(poi.getWritableConstantValue.get().toEpochMilli)
        _ => constant
      ...
      case pi: PrimitiveObjectInspector => pi match {
        ...
        case x: DateObjectInspector =>
          data: Any => {
            if (data != null) {
              DateTimeUtils.fromJavaDate(x.getPrimitiveJavaObject(data).toEpochMilli)
            } else {
              null
            }
          }
        case x: TimestampObjectInspector if x.preferWritable() =>
          data: Any => {
            if (data != null) {
              DateTimeUtils.fromJavaTimestamp(x.getPrimitiveWritableObject(data).getTimestamp.toSqlTimestamp)
            } else {
              null
            }
          }
